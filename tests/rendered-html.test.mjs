...
test("keeps the prototype self-contained", async () => {
  ...
  assert.match(wasmSimulation, /WASM_RUNTIME_VERSION = 10/);
  ...
  assert.match(wasmSimulation, /set_tech_corrosive_wake/);
  assert.match(wasmSimulation, /get_corrosive_wake_count/);
  ...
  assert.match(saveState, /SaveStateVersion\.V3/);
  ...
  assert.match(techTree, /CORROSIVE_WAKE_COST = 50_000/);
